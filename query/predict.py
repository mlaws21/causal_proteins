...
def calc_effect(intervention_data_filename, full_data_filename="final_full.csv", spline_filename="spline.pkl", treatment="Align_Score", num_boostraps=200):
    ...
    point_probs_cancer = np.array([spline_loaded(F(i, full_data["Align_Score"]), 1) for i in intervention_data[treatment]])
    ...
    for i in range(num_boostraps):
        ...
        boot_probs_cancer = np.array([spline_loaded(F(x, full_data["Align_Score"]), 1) for x in bootstrap_data[treatment]])
